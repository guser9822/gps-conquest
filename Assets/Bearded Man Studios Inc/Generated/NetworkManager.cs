...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] NetworkTileGenNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is NetworkTileGenNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (NetworkTileGenNetworkObject.Length > 0 && NetworkTileGenNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(NetworkTileGenNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateNetworkTileGen instead, its shorter and easier to type out ;)")]
		public NetworkTileGenBehavior InstantiateNetworkTileGenNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkTileGenNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkTileGenBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<NetworkTileGenBehavior>().networkObject = (NetworkTileGenNetworkObject)obj;
		...
		public NetworkTileGenBehavior InstantiateNetworkTileGen(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(NetworkTileGenNetworkObject[index]);
			var netBehavior = go.GetComponent<NetworkTileGenBehavior>();
